...
import {
  fetchTradeAnnouncementDetail,
  fallbackTradeAnnouncements,
  getTradeAnnouncementById,
} from "@/lib/tradeAnnouncements";

export const dynamic = "force-dynamic";

export function generateStaticParams() {
  return fallbackTradeAnnouncements.map((item) => ({ id: item.id }));
}

export async function generateMetadata(props: PageProps<"/announcement/[id]">) {
  const { id } = await props.params;
  const announcement = await getTradeAnnouncementById(decodeURIComponent(id));

  return {
    title: announcement ? `${announcement.title} - 全国政采云服务云平台` : "平台公告 - 全国政采云服务云平台",
    description: announcement?.title,
  };
}

export default async function AnnouncementPage(props: PageProps<"/announcement/[id]">) {
  const { id } = await props.params;
  const announcement = await getTradeAnnouncementById(decodeURIComponent(id));
  ...
  const detail = await fetchTradeAnnouncementDetail(announcement);
    ...
    <div className="min-h-screen bg-white text-[#111827]">
      <div className="border-b border-gray-200 bg-white text-xs text-[#222]">
        <div className="mx-auto flex h-9 w-full max-w-[1200px] items-center justify-between px-4">
          ...
          <div className="flex w-full items-center justify-between gap-4 sm:w-auto sm:justify-end">
            ...
            <span className="hidden md:inline">平台公告</span>
      ...
      <nav className="bg-[#3991f6]">
        <div className="mx-auto flex h-[53px] w-full max-w-[1200px] items-center overflow-x-auto px-4 text-[15px] font-semibold text-white">
          {["首页", "供应商入驻", "平台公告", "采购大厅", "直采大厅", "服务中心", "付费指导"].map((item, index) => (
            <Link
              key={item}
              href={index === 0 ? "/" : "#"}
              className={`flex h-full shrink-0 items-center px-4 hover:bg-[#1871d7] ${index === 2 ? "border-b-4 border-[#ffd900]" : ""}`}
            >
              {item}
            </Link>
          ))}
      ...
      <main className="mx-auto w-full max-w-[1200px] px-4 py-4">
        <div className="mb-3 text-sm text-[#333]">
          当前位置：<Link href="/" className="hover:text-[#3991f6]">首页</Link> &gt; 平台公告 &gt; 详情
        </div>
