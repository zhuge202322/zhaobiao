...
import { fetchYgcgNoticeDetail } from "@/lib/ygcgNotices";

export const dynamic = "force-dynamic";

export async function generateMetadata(props: PageProps<"/ygcg-notice/[code]">) {
  const { code } = await props.params;
  const notice = await fetchYgcgNoticeDetail(decodeURIComponent(code));
...
export default async function YgcgNoticePage(props: PageProps<"/ygcg-notice/[code]">) {
  const { code } = await props.params;
  const notice = await fetchYgcgNoticeDetail(decodeURIComponent(code));
    ...
    <div className="min-h-screen bg-white text-[#111827]">
      <div className="border-b border-gray-200 bg-white text-xs text-[#222]">
        <div className="mx-auto flex h-9 w-full max-w-[1200px] items-center justify-between px-4">
          ...
          <div className="flex w-full items-center justify-between gap-4 sm:w-auto sm:justify-end">
            ...
            <span className="hidden md:inline">政企采购电子卖场</span>
      ...
      <nav className="bg-[#3991f6]">
        <div className="mx-auto flex h-[53px] w-full max-w-[1200px] items-center overflow-x-auto px-4 text-[15px] font-semibold text-white">
          {["首页", "供应商入驻", "平台公告", "政企采购电子卖场", "央国企采购", "服务中心", "付费指导"].map((item, index) => (
            <Link
              key={item}
              href={index === 0 ? "/" : "#"}
              className={`flex h-full shrink-0 items-center px-4 hover:bg-[#1871d7] ${index === 3 ? "border-b-4 border-[#ffd900]" : ""}`}
            >
              {item}
            </Link>
          ))}
      ...
      <main className="mx-auto w-full max-w-[1200px] px-4 py-4">
        <div className="mb-3 text-sm text-[#333]">
          当前位置：<Link href="/" className="hover:text-[#3991f6]">首页</Link> &gt; 政企采购电子卖场 &gt; 详情
        </div>
        ...
        <section className="border border-[#d8d8d8] bg-white">
          ...
          <div className="px-6 pb-6 pt-2 text-center">
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-x-10 gap-y-2 text-sm text-gray-500">
              {notice.companyName && <span>信息来源：{notice.companyName}</span>}
              {notice.projectCode && <span>项目编号：{notice.projectCode}</span>}
              {notice.category && <span>采购类别：{notice.category}</span>}
              {notice.mode && <span>采购方式：{notice.mode}</span>}
            </div>
          </div>
        </section>
